...
public class OWLAxiomToStatedRelationshipService {
	...
	public void convertAxiomsToStatedRelationships(InputStreamSet snapshotFiles, InputStream deltaFile, OutputStream statedRelationshipOutputStream) throws ReleaseImportException, ConversionException, IOException {
		...
		try (BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(statedRelationshipOutputStream))) {
			...
			doConvert(snomedTaxonomy, writer, dummyRelationshipId, conversionService, snomedTaxonomy.getConceptAxiomMap(), true);
			doConvert(snomedTaxonomy, writer, dummyRelationshipId, conversionService, snomedTaxonomy.getInactiveConceptAxiomMap(), false);
		}
	}

	private void doConvert(SnomedTaxonomy snomedTaxonomy, BufferedWriter writer, Long dummyRelationshipId, AxiomRelationshipConversionService conversionService,
			Map<Long, List<OWLAxiom>> conceptAxiomMap, boolean active) throws ConversionException, IOException {

		System.out.printf("\n\nConverting %s axioms for %s concepts.\n\n", active ? "active" : "inactive", conceptAxiomMap.size());
		...
		for (Long conceptId : conceptAxiomMap.keySet()) {
			...
			for (Integer groupNumber : conceptRelationships.keySet()) {
				for (Relationship relationship : conceptRelationships.get(groupNumber)) {
					...
					// active
					writer.write(active ? "1" : "0");// Only active axioms can be converted.
					writer.write(TAB);
